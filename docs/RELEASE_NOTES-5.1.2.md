...
- **Desktop Flutter:** **5.1.2+6** — bundles include updated `arqma_flutter_solo_pool` from [ArqTras/FFI](https://github.com/ArqTras/FFI/releases/tag/1.0.15). Windows CI: MSVC coroutine fix (`local_auth_windows`); Inno Setup includes wallet FFI flat + `lib/` mirror; GUI footer reads version from `pubspec.yaml`; improved MinGW DLL preload for Inno installs (Win32 1114).
